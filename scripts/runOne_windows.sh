...
conc=high
    echo ${conc}
    for mic in `ls ${DIR}${conc}/Rep1/combined/5MIC* | awk -F'[/]' '{print $12}'`
    do
        output_dir=$(echo ${mic} | awk -F'[.]' '{print $1}')
        ./albatradis -v -c 1 -o ${conc}${output_dir} /Users/sbastkow/Projects/SoftwareProjects/AlbaTradis/Data/reference_BW25113.embl "${DIR}${conc}/Rep1/combined/${mic}" "${DIR}${conc}/Rep2/combined/${mic}" ${control1} ${control2}
    done;
